...
**Visual:** Close on the cell's outer membrane. A small attractant receptor sits on the surface. Faint glowing dots of sugar molecules drift through the water. One bumps the receptor — the receptor changes shape, a pulse travels inward. Pull back to the cell's path: when the sugar gradient rises, the flagella bundle together and the cell swims forward in a straight line — a *run*. When the gradient drops, the flagella unbundle, the cell tumbles, points somewhere new. Over many cycles, the cell climbs the gradient toward the food.

**Narration:** First behavior: finding food. Around the cell, faint molecules of sugar drift through the water. The cell can't see them. But on its surface, it has tiny chemical receptors — and when a sugar molecule bumps one, the receptor changes shape. That change reaches the flagella, and the flagella respond. When the sugar gets stronger, the cell swims forward — a *run*. When the sugar gets weaker, the flagella tumble it, pointing it somewhere new. Run, tumble, run, tumble — and slowly, somehow, the cell finds its way to the food.

#### Beat 4 — Avoiding Danger

**Visual:** A different scene. The water is darker. A toxic chemical drifts in from one side — visualized as a sickly green cloud. On the cell's surface, a *different* receptor — a repellent receptor — picks up the toxin. The cell's behavior: when the toxic concentration rises (the cloud getting closer), the cell tumbles, chaotic and undirected. When the concentration falls (the cloud receding), the cell runs forward. Over many cycles, the cell escapes the toxic zone.

**Narration:** Second behavior: avoiding danger. Different chemical now — a toxin this time. Different receptor on the surface, picking up something the cell needs to escape. And what we see... looks different. When the toxin gets stronger, the cell tumbles — chaos, no commitment to any direction. When the toxin gets weaker, the cell runs forward, away from the threat. A different strategy for a different problem. Or so it looks.

#### Beat 5 — Same Two Moves

**Visual:** Split screen. Left side: the food-seeking sequence, slowed down. Right side: the danger-avoidance sequence, slowed down. Both clips fade simultaneously. Underneath, two stripped-down motion diagrams appear — *run* (a straight forward line) and *tumble* (a random reorientation). The same two diagrams sit beneath both behaviors. A held beat. The diagrams line up, identical.

**Narration:** But watch what the cell is actually doing, on either side. In the food story: swim, swim, swim — pause — turn. In the danger story: turn — pause — swim, swim, swim. Strip away the labels — *food*, *danger* — and the two moves are the same. *Run.* *Tumble.* The cell has only ever known how to do these two things. It is using the same two-step dance for both problems. Biologists have a name for this dance. They call it *run and tumble.* It has been the cell's only strategy, for everything, for around three billion years.
