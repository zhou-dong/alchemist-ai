...
**The Nobel Prize connection — **Geoffrey Hinton shared the Nobel Prize in Physics in 2024 with John Hopfield, for foundational work enabling machine learning with neural networks. The RL story has its own recognition, of a different kind: the 2024 Nobel Prize in Chemistry went to Demis Hassabis and John Jumper, alongside David Baker, for AlphaFold2's protein structure prediction — a deep learning system, though trained by supervised learning rather than reinforcement learning. The broader point still holds: the systems Vera's nervous system anticipated are now woven into the fabric of scientific discovery itself.
